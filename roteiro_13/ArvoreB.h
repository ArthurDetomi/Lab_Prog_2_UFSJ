/*---------------- File: ArvoreB.h  --------------------+
|TAD: Arvore B                                          |
| | | Baseado no material do Prof. Rafael Sachetto	        | | Implementado
por Guilherme C. Pena em 02/12/2023      |
+-------------------------------------------------------+ */
...
void insereRec(Registro reg, Apontador ap, int *Cresceu, Registro *regRetorno,
               Apontador *apRetorno) {
  int i = 1, j;
  Apontador apTemp;
...
void removeRec(int Ch, Apontador *ap, int *Diminuiu) {
  ...
  while (i < Pag->n && Ch > Pag->r[i - 1].chave)
    i++;
  ...
  if (Ch == Pag->r[i - 1].chave) {
    ...
    if (*Diminuiu)
      Reconstitui(Pag->p[i - 1], *ap, i - 1, Diminuiu);
