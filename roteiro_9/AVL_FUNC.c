...
void em_ordem(NO *raiz, int nivel) {
  if (raiz != NULL) {
    ...
    printf("[%s, %.2lf,%d, %d, %d] ", raiz->info.nome, raiz->info.salario,
           raiz->fb, nivel, raiz->alt);
...
int buscar_com_maior_salario(AVL *raiz, Funcionario *f) {
  ...
  return buscar_com_maior_salario(&(*raiz)->dir, f);
}
...
int buscar_com_menor_salario(AVL *raiz, Funcionario *f) {
  ...
  return buscar_com_menor_salario(&(*raiz)->esq, f);
}
...
int buscar_elem_por_nome(AVL *raiz, char *nome, Funcionario *f) {
  ...
  return buscar_elem_por_nome(&(*raiz)->dir, nome, f) ||
         buscar_elem_por_nome(&(*raiz)->esq, nome, f);
}