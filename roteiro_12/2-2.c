...
int main() {
  ...
  int (*funcoes_chave[])(int, int) = {&chaveDivisao, &chaveMultiplicacao,
                                      &chaveDobra};
  char *funcoes_chave_nome[] = {"Divisão", "Multiplicação", "Dobra"};
  char *funcoes_sondagem_nomes[] = {"Sondagem Linear", "Sondagem Quadratica",
                                    "Sondagem Duplo Hash"};

  for (int fn_sd_idx = 0; fn_sd_idx < QTD_SONDAGENS; fn_sd_idx++) {
    const int INC = 3;

    Hash *hash_table = criaHash(n + INC);

    for (int j = 0; j < n; j++) {
      insereHash_EnderAberto(hash_table, array[j], funcoes_chave[DIVISAO],
                             fn_sd_idx);
    }

    printf("-------------------------------------\n");
    printf("Função hash aplicada : %s\n", funcoes_chave_nome[DIVISAO]);
    printf("Função de sondagem aplicada: %s\n",
           funcoes_sondagem_nomes[fn_sd_idx]);
